...
- We will simulate a quantitative trait with a known genetic effect
...
maf_pruned <- maf[snps_pruned]
ix <- which(!is.na(maf_pruned) & maf_pruned > 0.2 & maf_pruned < 0.3)[1]
```

## Phenotype & covariates 

```{r}
G1 <- GWASTools::getGenotypeSelection(geno_data, snpID = snps_pruned[ix], scanID = samp.keep.id)
G1 <- as.numeric(G1) 
...
```{r}
# Align PCs to scanID order to avoid misalignment
pcs_df <- data.frame(
  scanID = pcair_out$sample.id,
  PC1 = pcair_out$vectors[, 1],
  PC2 = pcair_out$vectors[, 2],
  PC3 = pcair_out$vectors[, 3],
  PC4 = pcair_out$vectors[, 4]
)

scanDF <- dplyr::left_join(
  data.frame(scanID = samp.keep.id, y = y, age = age, sex = sex),
  pcs_df,
  by = "scanID"
)
...
# Close the earlier reader and open a fresh GenotypeData without attaching scanAnnot.
# This avoids a length mismatch if the GDS contains samples not in scanAnnot2.
GWASTools::close(geno_reader)
geno <- GWASTools::GenotypeData(GWASTools::GdsGenotypeReader(gdsfile))

# Filter the genotype object to the analysis set
# Prefer creating a filtered view of the genotype object.
# Be tolerant to potential argument name variants across GWASTools versions.
filter_geno_safe <- function(g, scan_ids, snp_ids) {
  argn <- try(names(formals(GWASTools::GenotypeDataFilter)), silent = TRUE)
  if (inherits(argn, "try-error")) return(g)
  dots <- list(g, verbose = FALSE)
  # attach scan include
  if ("scan.include" %in% argn) dots$`scan.include` <- scan_ids
  if ("scanInclude" %in% argn)  dots$`scanInclude`  <- scan_ids
  # attach snp include
  if ("snp.include" %in% argn)  dots$`snp.include`  <- snp_ids
  if ("snpInclude" %in% argn)   dots$`snpInclude`   <- snp_ids
  do.call(GWASTools::GenotypeDataFilter, dots)
}

geno <- filter_geno_safe(geno, scanDF$scanID, snps_pruned)
```
...
- First fit the mixed model under the null hypothesis that each SNP has no effect
- Null model contains all of the covariates, ancestry representative PCs, random effects such as a polygenic effect due to genetic relatedness
- It does not include SNP genotype terms as fixed effects
