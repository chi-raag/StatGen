...
- KING is method for estimating pairwise kinship robust to population structure

## Relatedness 

```{r, label="king_code", echo = T}
# KING-robust pairwise relatedness on pruned SNPs
king <- snpgdsIBDKING(g, 
                      sample.id = samp.keep.id, 
                      snp.id = snps_pruned, 
                      type = "KING-robust")

# Convert to a symmetric kinship matrix for GENESIS
...
```{r, echo=FALSE}
# Summarize number of relative pairs above common thresholds
km <- KINGmat[rownames(KINGmat) %in% samp.keep.id, colnames(KINGmat) %in% samp.keep.id]
kin_vals <- km[upper.tri(km, diag = FALSE)]
rel_summary <- data.frame(
    "> 0.125 (2nd deg)" = sum(kin_vals > 0.125),
    "0.0625–0.125 (3rd deg)" = sum(kin_vals > 0.0625 & kin_vals <= 0.125)
)
knitr::kable(rel_summary)
```
...
cat(sprintf("Singletons (no relatives above threshold): %d\n", sum(family_summary$n_members == 1)))
```

## Ancestry PCs

- We want ancestry PCs for covariates
- Relatedness can confound PCA because family structure can dominate top PCs
- **PC‑AiR** (Conomos et al., 2015) computes PCs on unrelateds, then projects relateds
...
```{r, label="pcair_code"}
snpgdsClose(g)
geno_reader <- GWASTools::GdsGenotypeReader(gdsfile)
geno_data <- GenotypeData(geno_reader)
pcair_out <- pcair(geno_data, 
                   kinobj = KINGmat, 
                   divobj = KINGmat, 
...
## Step 5 — Ancestry-aware kinship (PC‑Relate) (code)

```{r, label="pcrel_code", eval=FALSE}
geno_data <- GenotypeBlockIterator(geno_data, snpInclude = snps_pruned)
pc_relate <- pcrelate(geno_data,
                      pcs = pcair_out$vectors[,1:2],
                      sample.include = samp.keep.id,
...
## Step 5 — Ancestry-aware kinship (explain)

**PC‑Relate**
- Uses **individual‑specific allele frequencies** inferred from PCs.  
- Produces **kinship** and **IBD** estimates robust to stratification.

**Why we need it**
- Mixed models require a covariance \( \sigma_g^2 K \); **PC‑Relate** gives a \(K\) that isn’t biased by ancestry differences.

**Deliverable**
- `K`: a GRM to plug into a linear mixed model.

---

## Step 5 — Ancestry-aware kinship (output)

```{r, ref.label="pcrel_code", echo=FALSE, eval = F}
```
...
**Model intent**
- Covariates: `age`, `sex`, `PC1–PC4` (ancestry).  
- Genetic effect: ~0.30 SD per minor allele on a standardized SNP.

**Deliverable**
- `geno` now contains sample annotations with \(y, X\), and PCs.
...
**LMM**  
\[
y = X\beta + \gamma G + u + \varepsilon,\quad
u \sim \mathcal{N}(0,\sigma_g^2 K),\quad \varepsilon \sim \mathcal{N}(0,\sigma_e^2 I).
\]
...
**Deliverables**
- `nullmod`: object with \(\hat\sigma_g^2,\hat\sigma_e^2\), fixed effects, and residuals.  
...
- **Calibration**: \( \lambda_{GC} \) near 1 indicates good control; large values suggest stratification, relatedness, or batch effects.

**Deliverables**
- `res`: per‑SNP estimates and p‑values.  
...
- **Manhattan**: look for peaks spanning multiple SNPs in LD; single‑SNP spikes can be artifacts.  
- **QQ**: early alignment with the diagonal + tail deviation at top hits is typical of a polygenic trait.  
- Show \( \lambda_{GC} \) on the figure or caption.

**Caveats**
- Our dataset is **small** and **simulated**; the figures illustrate **method behavior**, not discovery.
...
- **Deliverables**: annotated code, QQ/Manhattan, top hits table, \(\lambda_{GC}\).
