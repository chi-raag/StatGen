...
## Sampling Distributions in Genetics
...
```{r}
# Simulate sampling from population with p = 0.3
set.seed(123)
true_p <- 0.3
sample_sizes <- c(20, 100, 500)
n_sims <- 1000

sampling_results <- map_dfr(sample_sizes, function(n) {
  p_hats <- replicate(n_sims, {
    # Sample n individuals (2n alleles total)
    alleles <- rbinom(n, 2, true_p)  # Each individual contributes 2 alleles
...
ggplot(sampling_results, aes(x = p_hat)) +
  ...
  geom_vline(xintercept = true_p, color = "red", linetype = "dashed", size = 1) +
  facet_wrap(~paste("n =", sample_size), scales = "free_y") +
  labs(title = "Sampling Distribution of p̂", 
       x = "Sample Allele Frequency (p̂)", y = "Density") +
  theme_minimal()

# Compare theoretical vs empirical standard errors
se_comparison <- sampling_results %>%
  group_by(sample_size) %>%
  summarise(
    empirical_se = sd(p_hat),
    theoretical_se = first(theoretical_se),
    empirical_mean = mean(p_hat)
  )

ggplot(se_comparison, aes(x = sample_size)) +
  geom_point(aes(y = empirical_se, color = "Empirical"), size = 3) +
  geom_point(aes(y = theoretical_se, color = "Theoretical"), size = 3) +
  geom_line(aes(y = theoretical_se, color = "Theoretical")) +
  labs(title = "Standard Error: Theory vs Practice",
       x = "Sample Size", y = "Standard Error of p̂") +
  scale_color_manual(values = c("Empirical" = "blue", "Theoretical" = "red")) +
  theme_minimal()
```

## Population vs Sample: Key Insights
...
## Sampling Distribution: Hardy-Weinberg Testing
...
- **Probabilities**: $P(AA) = \frac{1}{4}$, $P(Aa) = \frac{1}{2}$, $P(aa) = \frac{1}{4}$

:::

## Testing Segregation Ratios

Observed offspring from Aa × Aa cross: 240 total
- 180 dominant phenotype, 60 recessive phenotype
- **Expected under 3:1 ratio**: 180 dominant, 60 recessive
...
**Purpose & Results**
- **Use**: Test if unknown dominant is AA or Aa
- **Expected**: 1:1 ratio (50% each phenotype)
- **Observed**: 23 dominant, 27 recessive (n=50)

:::

```{r}
observed_testcross <- c(23, 27)
expected_testcross <- c(25, 25)  # 1:1 ratio
chi_sq_test <- sum((observed_testcross - expected_testcross)^2 / expected_testcross)
p_val_test <- pchisq(chi_sq_test, df = 1, lower.tail = FALSE)
cat("χ² =", chi_sq_test, ", p-value =", p_val_test)
```
...
**Mendel's observed data**: 556 F₂ offspring
